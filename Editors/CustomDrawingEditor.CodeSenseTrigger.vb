...
Namespace Editors
    ...
    Partial Public Class CustomDrawingEditor
        ...
        ' Timer for delayed CodeSense triggering
        Private pCodeSenseTriggerTimer As UInteger = 0
        ...
        ''' <remarks>
        ''' Triggers CodeSense immediately for certain characters (like period)
        ''' or after a delay for regular typing
        ''' </remarks>
        Private Sub CheckCodeSenseTrigger(vChar As Char)
            Try
                ...
                If pCodeSenseTriggerTimer <> 0 Then
                    GLib.Source.Remove(pCodeSenseTriggerTimer)
                    pCodeSenseTriggerTimer = 0
                End If
                ...
                Select Case vChar
                    ...
                    Case Else
                        ' For regular characters, trigger after a delay if typing stops
        ...
        ''' <summary>
        ''' Starts a timer to trigger CodeSense after a delay
        ''' </summary>
        Private Sub StartCodeSenseTimer()
            Try
                ' Get the delay from settings (default 500ms)
                Dim lDelay As UInteger = 500
                If pSettingsManager IsNot Nothing Then
                    lDelay = CUInt(pSettingsManager.CodeSenseDelay)
                End If
                
                ' Start the timer
                pCodeSenseTriggerTimer = GLib.Timeout.Add(lDelay, AddressOf OnCodeSenseTimer)
                
            Catch ex As Exception
                Console.WriteLine($"StartCodeSenseTimer error: {ex.Message}")
            End Try
        End Sub
        
        ''' <summary>
        ''' Timer callback to trigger CodeSense after typing stops
        ''' </summary>
        Private Function OnCodeSenseTimer() As Boolean
            Try
                ' Reset timer ID
                pCodeSenseTriggerTimer = 0
                
                ' Check if we should show CodeSense
            ...
            Catch ex As Exception
                Console.WriteLine($"OnCodeSenseTimer error: {ex.Message}")
                Return False
            End Try
        End Function
            ...
            Try
                ...
                Select Case lPreviousWord.ToLower()
                    ...
                    Case "dim", "private", "public", "protected", "friend", "shared", "readonly", "const"
                        ' Variable declaration keywords might trigger
                        StartCodeSenseTimer()
                End Select
        ...
        Public Sub CancelCodeSenseTrigger()
            ...
            Catch ex As Exception
                Console.WriteLine($"CancelCodeSenseTrigger error: {ex.Message}")
            End Try
        End Sub

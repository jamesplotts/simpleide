    ...
    Partial Public Class CustomDrawObjectExplorer
        ...
        Private Sub CreateToolbar()
            Try
                ...
                ' A click that lands on the toolbar's own background (not on any button/
                ' control) is otherwise never claimed by any child widget, so GTK delivers
                ' it to the nearest ancestor that owns an input window, which ends up being
                ' the main window itself. A double-click there is then apparently
                ' interpreted as equivalent to double-clicking the titlebar (toggling
                ' maximize/restore) by this desktop's window manager. Explicitly marking the
                ' event handled here stops it from ever reaching that point.
                ...
                ' Toolbar canvas outside every ToolItem - anything landing within a
                ' ToolItem's own bounds but not claimed by a deeper interactive child (a
                ' button's own padding, the plain "Scale:" Label) never reaches them.
                ' EventBox exists specifically to give an area a real window so it can
                ' reliably claim events like this; wrapping the whole toolbar in one
                ' guarantees the same swallow-handlers actually see everything not already
                ' claimed by a button/combo's own window, without interfering with those -
                ' GTK still delivers directly to the most specific windowed widget under the
                ' cursor first.
                '
        ...
        Private Sub CreateScaleControls()
            Try
                ...
                ' Add scale label - a plain Gtk.Label requests/handles no events of its own by
                ' default, so an unclaimed click-and-drag on it would bubble up and be
                ...
                lScaleLabel.Events = Gdk.EventMask.ButtonPressMask Or Gdk.EventMask.PointerMotionMask
                AddHandler lScaleLabel.ButtonPressEvent, AddressOf OnToolbarBackgroundButtonPress
                AddHandler lScaleLabel.MotionNotifyEvent, AddressOf OnToolbarBackgroundMotion
        ...
        ''' site in CreateToolbar for why this exists
        ''' </summary>
